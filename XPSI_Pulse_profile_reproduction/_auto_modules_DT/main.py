""" Main module for NICER Synthetic Source <- X-PSI 2.1.0 PDT + NSX-H"""
import os
import argparse
import re
...
class GenerateConfigAction(argparse.Action):
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        ...
        with open('/Users/dc1408/PhD_research/Cross_checks_paper/Cross_checks/Zenodo/XPSI_Pulse_profile_reproduction/_auto_modules_DT/config.ini', 'w') as file:
...
parser = ArgumentParserCustom(
    description="""
    Main module for X-PSI PDT + NSX-H modelling of NICER Synthetic Source event data.
...
parser.add_argument('--p-cede-radius-bounds',
                    type=str,
                    nargs=2,
                    action=CompileAction,
                    help='Bounds of hot region "p" cede-member angular radius (radians). If no bounds are given (``None``), and no value is given (``None``), bounds default to the source code strict bounds.',
                    comment=True,
                    comment_line_above='rule')

parser.add_argument('--p-cede-radius-value',
                    type=str,
                    action=CompileAction,
                    help='Value of hot region "p" cede-member angular radius (radians). No value means the parameter is free, whilst a value means the parameter is fixed (and the prior may need to be modified manually). If you want the parameter to be derived from other parameters in a complex way, manual modification of the main module is necessary, but we support functions of one parameter in the configuration file. If the parameter is derived from one other parameter, e.g., the temperature of a hot region is derived from the temperature of the other hot region, then the value needs to be written using the following template: lambda x: f(x), "parameter", "space". In this template: f(x) is a function of the parameter x from which the value is derived; "parameter" is the name of the parameter x as a string; and "space" is an object in the global namespace, with name written as a string, which is a (sub)space of parameters from which the current value of parameter x can be accessed via getitem magic using "parameter".',
                    comment=True,
                    empty_lines_below=2)

parser.add_argument('--p-cede-temperature-bounds',
                    type=str,
                    nargs=2,
                    action=CompileAction,
                    help='Bounds of hot region "p" cede-member log10(temperature [K]). If no bounds are given (``None``), and no value is given (``None``), bounds default to the source code strict bounds.',
                    comment=True,
                    comment_line_above='rule')

parser.add_argument('--p-cede-temperature-prior',
                    type=str,
                    nargs='*',
                    action=NullAction,
                    help='Prior inverse CDF of hot-region p ceding region log10(temperature [K]). Supply a function of one variable (the probability mass ``x``), in the form of an expression that can be evaluated with the ``eval(...)`` builtin function, i.e., scipy.stats.truncnorm(x, ...). Note that the prior default PDF is uniform (with compact support), so do not supply a CDF if a uniform prior is desired, or to be explicit, use: DEFAULT UNIFORM. You must use DEFAULT UNIFORM to overwrite a default CDF shown in the auto-generated configuration file, unless the parameter is fixed/derived in which case the prior flag is silently ignored. You can also use the flag more than once: the last usage must be an expression that will be dynamically evaluated using the ``eval(...)`` builtin and must return a float to set as the parameter value; the other usages can be helper statements executed with the ``exec(...)`` builtin, e.g., to set temporary local variables to make the code (and configuration file more readable).',
                    comment=True)

parser.add_argument('--p-cede-temperature-value',
                    type=str,
                    action=CompileAction,
...
parser.add_argument('--p-cede-colatitude-bounds',
                    type=str,
                    nargs=2,
                    action=CompileAction,
                    help='Bounds of hot region "p" cede-member colatitude w.r.t stellar spin axis (radians). If no bounds are given (``None``), and no value is given (``None``), bounds default to the source code strict bounds.',
                    comment=True,
                    comment_line_above='rule')

parser.add_argument('--p-cede-colatitude-value',
                    type=str,
                    action=CompileAction,
...
parser.add_argument('--p-cede-azimuth-bounds',
                    type=str,
                    nargs=2,
                    action=CompileAction,
                    help='Bounds of hot region "p" cede-member azimuth relative to super-member (radians). If no bounds are given (``None``), and no value is given (``None``), bounds default to the source code strict bounds.',
                    comment=True,
                    comment_line_above='rule')

parser.add_argument('--p-cede-azimuth-value',
                    type=str,
                    action=CompileAction,
...
if __name__ == '__main__':
    ...
    if xpsi._verbose:
        print('Parsing configuration file...')
    args, _ = parser.parse_known_args(['@/Users/dc1408/PhD_research/Cross_checks_paper/Cross_checks/Zenodo/XPSI_Pulse_profile_reproduction/_auto_modules_DT/config.ini'])
    if xpsi._verbose:
        print('Configuration file parsed.')
...
bounds['cede_radius'] = parse_bounds(args.p_cede_radius_bounds,
                                     args.p_cede_radius_value)

values['cede_radius'] = parse_value(args.p_cede_radius_value)

bounds['cede_temperature'] = parse_bounds(args.p_cede_temperature_bounds,
                                          args.p_cede_temperature_value)

values['cede_temperature'] = parse_value(args.p_cede_temperature_value)

bounds['cede_colatitude'] = parse_bounds(args.p_cede_colatitude_bounds,
                                         args.p_cede_colatitude_value)

values['cede_colatitude'] = parse_value(args.p_cede_colatitude_value)

bounds['cede_azimuth'] = parse_bounds(args.p_cede_azimuth_bounds,
                                      args.p_cede_azimuth_value)

values['cede_azimuth'] = parse_value(args.p_cede_azimuth_value)
