""" Main module for NICER Synthetic Source  <- X-PSI 2.1.0 PST + NSX-H"""
import os
import argparse
import re
...
parser = ArgumentParserCustom(
    description="""
    Main module for X-PSI PST + NSX-H modelling of NICER Synthetic Source  event data.
...
parser.add_argument('--p-omit-colatitude-bounds',
                    type=str,
                    nargs=2,
                    action=CompileAction,
                    help='Bounds of hot region "p" omit-member colatitude w.r.t stellar spin axis (radians). If no bounds are given (``None``), and no value is given (``None``), bounds default to the source code strict bounds.',
                    comment=True,
                    comment_line_above='rule')

parser.add_argument('--p-omit-colatitude-value',
                    type=str,
                    action=CompileAction,
                    help='Value of hot region "p" omit-member colatitude w.r.t stellar spin axis (radians). No value means the parameter is free, whilst a value means the parameter is fixed (and the prior may need to be modified manually). If you want the parameter to be derived from other parameters in a complex way, manual modification of the main module is necessary, but we support functions of one parameter in the configuration file. If the parameter is derived from one other parameter, e.g., the temperature of a hot region is derived from the temperature of the other hot region, then the value needs to be written using the following template: lambda x: f(x), "parameter", "space". In this template: f(x) is a function of the parameter x from which the value is derived; "parameter" is the name of the parameter x as a string; and "space" is an object in the global namespace, with name written as a string, which is a (sub)space of parameters from which the current value of parameter x can be accessed via getitem magic using "parameter".',
                    comment=True,
                    empty_lines_below=2)

parser.add_argument('--p-omit-azimuth-bounds',
                    type=str,
                    nargs=2,
                    action=CompileAction,
                    help='Bounds of hot region "p" omit-member azimuth relative to super-member (radians). If no bounds are given (``None``), and no value is given (``None``), bounds default to the source code strict bounds.',
                    comment=True,
                    comment_line_above='rule')

parser.add_argument('--p-omit-azimuth-value',
                    type=str,
                    action=CompileAction,
                    help='Value of hot region "p" omit-member azimuth relative to super-member (radians). No value means the parameter is free, whilst a value means the parameter is fixed (and the prior may need to be modified manually). If you want the parameter to be derived from other parameters in a complex way, manual modification of the main module is necessary, but we support functions of one parameter in the configuration file. If the parameter is derived from one other parameter, e.g., the temperature of a hot region is derived from the temperature of the other hot region, then the value needs to be written using the following template: lambda x: f(x), "parameter", "space". In this template: f(x) is a function of the parameter x from which the value is derived; "parameter" is the name of the parameter x as a string; and "space" is an object in the global namespace, with name written as a string, which is a (sub)space of parameters from which the current value of parameter x can be accessed via getitem magic using "parameter".',
                    comment=True,
                    empty_lines_below=2)
...
bounds['omit_colatitude'] = parse_bounds(args.p_omit_colatitude_bounds,
                                         args.p_omit_colatitude_value)

values['omit_colatitude'] = parse_value(args.p_omit_colatitude_value)

bounds['omit_azimuth'] = parse_bounds(args.p_omit_azimuth_bounds,
                                      args.p_omit_azimuth_value)

values['omit_azimuth'] = parse_value(args.p_omit_azimuth_value)
